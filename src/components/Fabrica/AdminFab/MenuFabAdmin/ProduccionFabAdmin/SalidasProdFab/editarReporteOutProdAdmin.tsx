...
import './editarReporteOutProdAdmin.css';
...
export const EditarReporteOutProdAdmin: React.FC<Props> = ({selectedRecord, changeViewStatus}) => {
  const [ selectedDateIn, setSelectedDateIn ] = useState<any>(selectedRecord?.datein);
  const [ selectedDateOut, setSelectedDateOut ] = useState<any>(selectedRecord?.dateout);
  const [ selectedApproved, setSelectedApproved ] = useState<any>('Falso');
  ...
  const updateRecord = (event: { preventDefault: () => void; }):void => {
    ...
    if (selectedApproved === 'Falso') {
      console.log('Updated ffor first time');
      const idNm = parseInt(selectedRecord?.id);
      const dateString = setSelectedDateOut.toString();
      const updateObj = {
        'approvedprodsalida': selectedApproved,
      ...
      console.log('Updated but waiting for approval:',selectedApproved);
      setIsDisable(true);
    }


    if (selectedApproved === 'Verdadero') {
      console.log('Updated for multiple time');
      const newTotal = selectedPiecesOut - selectedPieces;
      const idNm = parseInt(selectedRecord?.id);
      const dateString = setSelectedDateOut.toString();
      const updateObj = {
        'approvedprodsalida': selectedApproved,
  ...
  const createRecordOut = () => {
    createProdRecorductionOut({
      'approvedprodsalida': selectedApproved,
      'approvepord': 'Falso',
      'quesoname': selectedRecord?.quesoname,
      'quesostatus': 'Salida',
      'quesoavailability': false,
      'nopieces': parseInt(selectedPiecesOut),
      'nopiecessalida': parseInt(selectedPiecesOut),
      'weigthcurrent': parseInt(selectedKg),
      'datein': selectedRecord?.datein,
      'dateout': selectedDateOut.toString(),
      'price': parseInt(selectedPrice),
      'lotenum': selectedRecord?.lotenum,
      'refrigerationtype': selectedRecord?.refrigerationtype
    })
  }
      ...
      <div className="crearReporteProdAdmin">
        <form className="cheese-categories-form">
          <div className="crear-reporte-queso-marco">
            ...
            <h3>Pz Disponibles: {selectedPieces}</h3>
